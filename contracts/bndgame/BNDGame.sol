...
contract BnDGame is IBnDGame, Ownable, ReentrancyGuard, Pausable {
  ...
  function setContracts(address _gp, address _traits, address _bnd, address _tower, address _alter, address _rand) external onlyOwner {
    gpToken = IGP(_gp);
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
    ...
    uint256 paidTokens = bndNFT.getPaidTokens();
