...
contract Tower is ITower, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  modifier requireContractsSet() {
      require(address(bndNFT) != address(0) && address(gpToken) != address(0) 
        && address(bndGame) != address(0) && address(randomizer) != address(0), "Contracts not set");
      _;
  }

  function setContracts(address _bndNFT, address _gp, address _bndGame, address _rand) external onlyOwner {
    bndNFT = IBnD(_bndNFT);
    gpToken = IGP(_gp);
    bndGame = IBnDGame(_bndGame);
  ...
  function addManyToTowerAndFlight(address account, uint16[] calldata tokenIds) external override nonReentrant {
    require(tx.origin == _msgSender() || _msgSender() == address(bndGame), "Only EOA");
    require(account == tx.origin, "account to sender mismatch");
    for (uint i = 0; i < tokenIds.length; i++) {
      if (_msgSender() != address(bndGame)) { // dont do this step if its a mint + stake
        require(bndNFT.ownerOf(tokenIds[i]) == _msgSender(), "You don't own this token");
        bndNFT.transferFrom(_msgSender(), address(this), tokenIds[i]);
  ...
  function _claimWizardFromTower(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      bndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Wizard
    } else {
  ...
  function _claimDragonFromFlight(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      bndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // Send back Dragon
    } else {
  ...
  function rescue(uint256[] calldata tokenIds) external nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (bndNFT.isWizard(tokenId)) {
        ...
        delete tower[tokenId];
        numWizardsStaked -= 1;
        bndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Wizards
