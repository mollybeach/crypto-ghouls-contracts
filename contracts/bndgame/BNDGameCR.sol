...
contract BnDGameCR is IBnDGame, Ownable, ReentrancyGuard, Pausable {
  ...
  modifier requireContractsSet() {
      require(address(gpToken) != address(0) && address(traits) != address(0) 
        && address(bndNFT) != address(0) && address(tower) != address(0) && address(alter) != address(0)
        , "Contracts not set");
      _;
  }

  function setContracts(address _gp, address _traits, address _bnd, address _tower, address _alter) external onlyOwner {
    gpToken = IGP(_gp);
  ...
  function getGardendingMint(address addr) external view returns (MintCommit memory) {
    require(_pendingCommitId[addr] != 0, "no pending commits");
    return _mintCommits[addr][_pendingCommitId[addr]];
  }

  function hasMintGardending(address addr) external view returns (bool) {
  ...
  function reveal(address addr) internal {
    ...
    for (uint k = 0; k < commit.amount; k++) {
      ...
      if (!commit.stake || recipient != addr) {
        ...
        bndNFT.mint(address(tower), seed);
  ...
  function sacrifice(uint256 tokenId, uint256 gpAmt) external whenNotPaused nonReentrant {
    ...
    IBnD.WizardDragon memory nft = bndNFT.getTokenTraits(tokenId);
    uint16 minted = bndNFT.minted();
    uint256 maxTokens = bndNFT.getMaxTokens();
    uint256 gpMintCost = mintCost(minted, maxTokens);
    require(gpMintCost > 0, "Sacrificial alter currently closed");
