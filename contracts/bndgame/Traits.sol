...
contract Traits is Ownable, ITraits {
  ...
  function setBnD(address _bndNFT) external onlyOwner {
    bndNFT = IBnD(_bndNFT);
  }
  ...
  function drawSVG(uint256 tokenId) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(
      '<svg id="bndNFT" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= bndNFT.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    require(_msgSender() == address(bndNFT), "hmmmm what doing?");
    IBnD.WizardDragon memory s = bndNFT.getTokenTraits(tokenId);
