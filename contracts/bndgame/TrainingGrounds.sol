...
contract TrainingGrounds is ITrainingGrounds, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  uint16 public constant MAX_WHIP_EMISSION = 8000;
  uint16 public constant MAX_MAGIC_RUNE_EMISSION = 800;
  ...
  // reference to the BnD NFT contract
  IBnD public bndNFT;
  ...
  modifier requireContractsSet() {
      require(address(bndNFT) != address(0) && address(gpToken) != address(0) 
        && address(alter) != address(0), "Contracts not set");
      _;
  }

  function setContracts(address _bndNFT, address _gp, address _alter) external onlyOwner {
    bndNFT = IBnD(_bndNFT);
    gpToken = IGP(_gp);
  ...
  function ownsToken(uint256 tokenId) external view returns (bool) {
    uint64 lastTokenWrite = bndNFT.getTokenWriteBlock(tokenId);
    ...
    IBnD.WizardDragon memory s = bndNFT.getTokenTraits(tokenId);
    if(s.isWizard) {
  ...
  function addManyToTowerAndFlight(address tokenOwner, uint16[] calldata tokenIds) external override nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (bndNFT.ownerOf(tokenIds[i]) != address(this)) { // a mint + stake will send directly to the staking contract
        require(bndNFT.ownerOf(tokenIds[i]) == tokenOwner, "You don't own this token");
        bndNFT.transferFrom(tokenOwner, address(this), tokenIds[i]);
      } else if (tokenIds[i] == 0) {
        continue; // there may be gaps in the array for stolen tokens
      }

      if (bndNFT.isWizard(tokenIds[i])) 
  ...
  function addManyToTrainingAndFlight(uint256 seed, address tokenOwner, uint16[] calldata tokenIds) external override nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
        ...
        if(recipient != tokenOwner) { // stolen
            bndNFT.transferFrom(tokenOwner, recipient, tokenIds[i]);
            continue;
        }
        bndNFT.transferFrom(tokenOwner, address(this), tokenIds[i]);
        if (bndNFT.isWizard(tokenIds[i])) {
  ...
  function _claimWizardFromTower(uint256 tokenId, bool unstake, address tokenOwner) internal returns (uint256 owed) {
    require(bndNFT.ownerOf(tokenId) == address(this), "Doesn't own token");
    StakeWizard memory stake = tower[tokenId];
    ...
    if (unstake) {
        ...
        bndNFT.safeTransferFrom(address(this), stake.owner, tokenId, ""); // send back Wizard
    } else {
  ...
  function _claimWizardFromTraining(uint256 seed, uint256 tokenId, bool unstake, address tokenOwner) internal returns (uint16 owed) {
    ...
    if (unstake) {
        ...
        bndNFT.safeTransferFrom(address(this), recipient, tokenId, ""); // send back Wizard
    } else {
        training[tokenId] = StakeWizard({
            owner: stake.owner,
            tokenId: uint16(tokenId),
  ...
  function _claimDragonFromFlight(uint256 tokenId, bool unstake, address tokenOwner) internal returns (uint256 owedGP, uint16 owedRunes) {
    ...
    if (unstake) {
      ...
      bndNFT.safeTransferFrom(address(this), stake.owner, tokenId, ""); // Send back Dragon
    } else {
  ...
  function rescue(uint256[] calldata tokenIds) external nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (bndNFT.isWizard(tokenId)) {
        ...
        numWizardsStaked -= 1;
        bndNFT.safeTransferFrom(address(this), tx.origin, tokenId, ""); // send back Wizards
        ...
        delete flightIndices[tokenId]; // Delete old mapping
        _deposits[stake.owner].dragons.remove(tokenId);
        bndNFT.safeTransferFrom(address(this), tx.origin, tokenId, ""); // Send back Dragon
