...
contract Inferno is Ownable, ERC721Enumerable {
    ...
    //uints
    //anaconda Supply 
    uint256 public anacondaSupply = 1000;
    //anaconda total supply
    uint256 public anacondaTotalSupply;
    //witch supply
    ...
    function mintPigztaked(uint256 amount, uint256 tokenId) public returns (string memory) {
        require(amount >= berriesCost, "need more berries");
        require(!rollUsed[tokenId], "roll used!");
        require(started, "not started");
        require(totalPigz < anacondaSupply + witchSupply, "too many pigz");
        ...
        for (uint256 i; i < deposits.length; i++) {
            if (deposits[i] == tokenId) {
                ...
                if (rollOutcome == 1 && anacondaTotalSupply < 1000) {
                    _mint(_msgSender(), ++anacondaTotalSupply);
                    totalPigz++;
                    return "Heaven";
                }
                if (rollOutcome == 2 && witchTotalSupply < 8000) {
                    _mint(_msgSender(), anacondaSupply + ++witchTotalSupply);
                    totalPigz++;
                    return "Hell";
                }
                return "Nothing!";
