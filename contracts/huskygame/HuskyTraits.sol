...
contract HuskyTraits is Ownable, ITraits {
  ...
  IFleecef public huskie;

  constructor() {}

  /** ADMIN */

  function setFleecef(address _huskie) external onlyOwner {
  ...
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    IFleecef.SheepWolf memory s = huskie.getTokenTraits(tokenId);
    ...
    return string(abi.encodePacked(
      '<svg id="huskie" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= huskie.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isSheep ? 'Sheep #' : 'Wolf #',
      tokenId.toString(),
      '", "description": "Thousands of Sheep and Wolves compete on a farm in the metaverse. A tempting prize of $FLEECE awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
