...
contract DollPupClub is ERC721Enumerable, Ownable {
    ...
    uint256 public MAX_DOLL_CLAIM = 420;
    uint256 public DOLL_CLAIMS = 0;
    ...
    function setContracts(address _baycAddress, address _dollAddress, address _gmAddress) public onlyOwner {
        baycAddress = IERC721(_baycAddress);
        dollAddress = IERC721(_dollAddress);
    ...
    function dollClaim(uint256 tokenId) public {
        require(_presaleStarted, "presale not started");
        require(dollAddress.ownerOf(tokenId) == msg.sender, "Not the owner of this doll");
        require(DOLL_CLAIMS + 1 <= MAX_DOLL_CLAIM, "All Doll reservations claimed");
        require(CURRENT_SUPPLY + 1 <= totalCount, "Total supply reached");
        require(IERC721(_contractAddress).balanceOf(msg.sender) < CLAIM_MAX_WALLET, "Max claims reached");
        DOLL_CLAIMS++;
        emit Mint(_msgSender(), CURRENT_SUPPLY + 1, 1);
