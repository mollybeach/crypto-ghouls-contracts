...
contract SpookyCreatures is ERC721URIStorage, Ownable, Pausable {
    ...
    IERC721 public _bigGardensNFT;
    IStaking public _stakingContract;

    //constructor args 
    constructor() ERC721("SpookyCreatures", "HCreatures") {
    ...
    modifier isSetUp() {
        ...
        require(address(_bigGardensNFT) != address(0), "bigGardensNFT not set");
    ...
    function setContracts(address creature, address bigGarden, address stakingContract) external onlyOwner {
        _creatureNFT = IERC721(creature);
        _bigGardensNFT = IERC721(bigGarden);
    ...
    function mintHolderPresale(uint256 times) public payable canMint(times) whenNotPaused {
        ...
        require(_creatureNFT.balanceOf(msg.sender) > 0 
            || _bigGardensNFT.balanceOf(msg.sender) > 0
            || _stakingContract.depositsOf(msg.sender).length > 0,
            "Must hold a creature or bigGarden");
