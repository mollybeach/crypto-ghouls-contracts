...
contract SerumStaking is ERC1155, IERC721Receiver, Ownable, ReentrancyGuard, Pausable {
    ...
    function claimSerum() external whenNotPaused {
        ...
        for (uint256 i = 0; i < _deposits[_msgSender()].length(); i++) {
            ...
            SerumReward storage elixir = _tokenRewards[curToken];
            uint256 elixirCreationTime = elixir.startTime + timeToStakeForReward;
            if(elixir.fishDeposited >= fishFor100Boost) {
                // remove 100% of the maximum allowed elixir production time reduction.
                // Since the maximum reduction time is 50%, we divide by 2.
                elixirCreationTime = elixirCreationTime - (timeToStakeForReward / 2);
            }
            else if(elixir.fishDeposited >= fishFor50Boost) {
                // remove 50% of the maximum allowed elixir production time reduction.
                // We want 1/4 of the total time it takes because
                //  1/2 of 1/2 max production = 1/4
                elixirCreationTime = elixirCreationTime - (timeToStakeForReward / 4);
            }
            else if(elixir.fishDeposited >= fishFor25Boost) {
                // remove 25% of the maximum allowed elixir production time reduction.
                // We want 1/8 of the total time it takes because
                //  1/4 of 1/2 max production = 1/8
                elixirCreationTime = elixirCreationTime - (timeToStakeForReward / 8);
            }
            if(elixir.startTime > 0 && block.timestamp >= elixirCreationTime) {
                elixirs += 1;
                // Set the new start time to exactly when the last elixir was claimable
                elixir.startTime = elixirCreationTime;
                // If the gorilla was given more than was needed for 1 elixir,
                //  the gorilla didn't eat all of it and will eat them for the next elixir
                if(elixir.fishDeposited > fishFor100Boost) {
                    elixir.fishDeposited -= fishFor100Boost;
                }
                else {
                    elixir.fishDeposited = 0;
                }
            }
        }
        require(elixirs > 0, "No elixir to claim");
        _mint( _msgSender(), SERUM, elixirs, "");
    }
